...
test("replays widgets and statuses registered before an event listener attaches", () => {
  const { context, wrapper } = createContext();
  context.setWidget("rpiv-todos", ["Todos", "one"], { placement: "aboveEditor" });
  context.setStatus("progress", "ready");

  const replay = [];
  const unsubscribe = wrapper.onEvent((event) => replay.push(event));
  assert.deepEqual(replay.filter((event) => event.method === "setWidget").map((event) => [event.widgetKey, event.widgetLines]), [
    ["rpiv-todos", ["Todos", "one"]],
  ]);
  assert.deepEqual(replay.filter((event) => event.method === "setStatus").map((event) => [event.statusKey, event.statusText]), [
    ["progress", "ready"],
  ]);
  unsubscribe();
  wrapper.destroy();
