...
test("finds the live subagent process through the last assistant entry", async () => {
  const { findLiveSubagentProcessEnd } = await loadSubject();
  const messages = [
    { role: "user", content: "plan" },
    assistant([{ type: "text", text: "I'll delegate this." }]),
    assistant([{ type: "toolCall", toolCallId: "agent-1", toolName: "Agent", input: {} }]),
    { role: "toolResult", toolCallId: "agent-1", content: [] },
    assistant([{ type: "text", text: "Checking the agent's work." }]),
  ];
  assert.equal(findLiveSubagentProcessEnd(messages, 0, 3), 2);
  assert.equal(findLiveSubagentProcessEnd(messages, 0, messages.length), 4);
  assert.equal(findLiveSubagentProcessEnd(messages, 0, 2), -1);
  assert.equal(findLiveSubagentProcessEnd([
    { role: "user", content: "run bash" },
    assistant([{ type: "toolCall", toolCallId: "bash-1", toolName: "bash", input: {} }]),
  ], 0, 2), -1);
  assert.equal(findLiveSubagentProcessEnd([
    { role: "user", content: "first turn" },
    assistant([{ type: "toolCall", toolCallId: "agent-1", toolName: "Agent", input: {} }]),
    { role: "user", content: "second turn" },
    assistant([{ type: "text", text: "Not a subagent turn" }]),
  ], 2, 4), -1);
});
