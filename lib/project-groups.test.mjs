...
const {
  getProjectActivity,
  getRecentProjects,
  otherWorkspaceActivityIndicator,
  sessionsForProject,
...
test("collapsed selector distinguishes running work from completed unread work", () => {
  const activity = new Map([
    ["current", { running: 1, unread: 1 }],
    ["elsewhere", { running: 1, unread: 0 }],
  ]);
  assert.equal(otherWorkspaceActivityIndicator(activity, "current"), "running");
  activity.set("finished", { running: 0, unread: 1 });
  assert.equal(otherWorkspaceActivityIndicator(activity, "current"), "unread");
  activity.set("elsewhere", { running: 0, unread: 0 });
  assert.equal(otherWorkspaceActivityIndicator(activity, "current"), "unread");
  activity.delete("finished");
  assert.equal(otherWorkspaceActivityIndicator(activity, "current"), null);
});
