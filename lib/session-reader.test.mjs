...
const { listSessionFamilies, listVisibleSessionRows } = await jiti.import("./session-family.ts");
...
test("Pi Agent children survive rename while similarly named forks remain roots", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-native-agent-"));
  const parentPath = join(dir, "parent.jsonl");
  ...
  const header = (id, parentSession) => JSON.stringify({
    type: "session", version: 3, id, timestamp: created.toISOString(), cwd: dir,
    ...(parentSession ? { parentSession } : {}),
  }) + "\n";
  writeFileSync(parentPath, header("parent") + JSON.stringify({
    type: "custom", customType: "subagents:record", data: { id: "d180b602-944c-481", type: "worker" },
  }) + "\n");
  writeFileSync(childPath, header("worker", parentPath) + JSON.stringify({ type: "session_info", name: "worker#d180b602" }) + "\n" + JSON.stringify({ type: "session_info", name: "renamed child" }) + "\n");
  writeFileSync(forkPath, header("fork", parentPath) + JSON.stringify({ type: "session_info", name: "worker#deadbeef" }) + "\n");

  const originalListSessions = listSessions;
  listSessions = async () => Promise.all([parentPath, childPath, forkPath].map((path) => scanSessionFileInfo(path)));
  resetSessionListState();
  t.after(() => {
    listSessions = originalListSessions;
    resetSessionListState();
  ...
  const sessions = await listAllSessions({ force: true });
  assert.deepEqual(sessions.find((s) => s.id === "worker").relation, {
    kind: "subagent", parentSessionId: "parent", profile: "worker", description: "worker#d180b602",
  });
  assert.deepEqual(sessions.find((s) => s.id === "fork").relation, { kind: "fork", originSessionId: "parent" });
  const families = listSessionFamilies(sessions);
  assert.deepEqual(listVisibleSessionRows(families, new Set()).map((row) => row.session.id), ["parent", "fork"]);
  assert.deepEqual(listVisibleSessionRows(families, new Set(["parent"])).map((row) => row.session.id), ["parent", "worker", "fork"]);
});
