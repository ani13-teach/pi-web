...
test("keeps dead patterns it cannot attribute to exactly one model", async () => {
  const result = await plan([
    "typo-model",
    "*sonnet*",
    "*gateway/old-*",
    "KBQ/retired-*",
    "KBQ/[0.2]removed-model",
    "KBQ/retired-model",
  ]);

  assert.deepEqual(result.removed, ["KBQ/retired-model"]);
  assert.deepEqual(result.patterns.slice(0, 5), [
    "typo-model",
    "*sonnet*",
    "*gateway/old-*",
    "KBQ/retired-*",
...
test("leaves the whitelist alone when models.json defines no providers", async () => {
  const result = await plan(["KBQ/retired-model"], { definedModels: {} });
