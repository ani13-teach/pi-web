...
function logClassifierIo(decision: ClassifyResult, log: LogCtx): void {
  ...
  if (decision.io) {
    log.reasoning = decision.io.reasoning;
    log.classifierModel = decision.io.model;
  }
  if (!log.logger.enabled || !decision.io) return;

  const ios = [...(decision.io.previous ?? []), decision.io];
  for (const candidateIo of ios) for (const attempt of candidateIo.attempts) {
  ...
  log.logger.append({
    ...
    ...(decision.io.previous?.length ? { previous: decision.io.previous } : {}),
    parsed: {
    ...
    function inspectAutomode(
      ...
      if (action === "status") {
        const status = [
          ...
          `classifier fallbacks: ${cfg.classifierFallbackModels.join(", ") || "none"}`,
