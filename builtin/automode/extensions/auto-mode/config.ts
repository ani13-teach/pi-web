...
export function validateSettingsFile(
  ...
  if (settings.autoMode !== undefined) {
    if (
      ...
      if (
        hasOwn(autoMode, "classifierFallbackModels")
      ) {
        const value = autoMode.classifierFallbackModels;
        if (!Array.isArray(value)) {
          diagnostics.push(`${source}: autoMode.classifierFallbackModels must be an array of provider/model strings`);
        } else {
          for (const [index, entry] of value.entries()) {
            if (typeof entry !== "string" || !parseModelSpec(entry)) {
              diagnostics.push(`${source}: autoMode.classifierFallbackModels[${index}] must be a provider/model string`);
            }
          }
        }
      }
...
function validClassifierFallbackModels(value: unknown): value is string[] {
  return Array.isArray(value) && value.every((entry) => typeof entry === "string" && parseModelSpec(entry) !== undefined);
}
...
function applyAutoModeScalars(
  ...
  return {
    ...
    classifierFallbackModels: validClassifierFallbackModels(settings.classifierFallbackModels)
      ? [...settings.classifierFallbackModels]
      : base.classifierFallbackModels,
